    ...
    return (
        <ResponsiveContainer width='100%' minWidth={200} height={500}>
            <BarChart data={chartData}>
                ...
                <YAxis />
                <Tooltip />
                { shouldDisplayVolumes && <ReferenceLine y={feedStatsData.average_volume_per_day} stroke="red" label='μ oz' strokeDasharray="3 10" isFront={true} /> }
                { shouldDisplayVolumes && <ReferenceLine y={feedStatsData.average_volume_per_day + feedStatsData.volume_std} stroke="red" label='μ+σ oz' strokeDasharray="3 10" isFront /> }
                { shouldDisplayVolumes && <ReferenceLine y={feedStatsData.average_volume_per_day - feedStatsData.volume_std} stroke="red" label='μ-σ oz' strokeDasharray="3 10" isFront/> }
                { shouldDisplayTimes && <ReferenceLine y={feedStatsData.average_time_per_day} stroke="green" label='μ mins' strokeDasharray="3 10" isFront/> }
                { shouldDisplayTimes && <ReferenceLine y={feedStatsData.average_time_per_day + feedStatsData.time_std} stroke="green" label='μ+σ mins' strokeDasharray="3 10" isFront/> }
                { shouldDisplayTimes && <ReferenceLine y={feedStatsData.average_time_per_day - feedStatsData.time_std} stroke="green" label='μ-σ mins' strokeDasharray="3 10" isFront/> }
